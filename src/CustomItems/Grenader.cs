namespace vip.zeitvertreib.CustomItems
{
    ...
    public class Grenader : CustomWeapon
    {
        ...
        protected override void OnDropping(DroppingItemEventArgs ev)
        {
            ...
            CommandProcessor.ProcessAdminChat("[ZEITVERTREIB-COMMONS] " + ev.Player.DisplayNickname + " dropped dangerous weapon (Grenader)", new ServerConsoleSender());
        }

        protected override void OnPickingUp(PickingUpItemEventArgs ev)
        {
            Map.Broadcast(10, "<size=40><color=#F00>DANGER: A WEAPON BELONGING TO \n max.bambus76 WAS PICKED UP</color></size>\n", BroadcastFlags.Normal, true);
            CommandProcessor.ProcessAdminChat("[ZEITVERTREIB-COMMONS] " + ev.Player.DisplayNickname + " picked up dangerous weapon (Grenader)", new ServerConsoleSender());
